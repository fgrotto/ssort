 ...
 *
 * BMSSP (Duan et al., Algorithm 3) is run end-to-end on the constant-degree
 * transformed graph (paper §2) and is exact (Lemma 3.7): the returned U
 * equals T_{<B}(S) and is complete, so no final Dijkstra pass is needed.
 */
...
/* Total-order label of a vertex: (length, #vertices, endpoint id).
   Mirrors the paper's path tuple ⟨len, α, v_α,…,v_1⟩: for labels of two
   distinct vertices the reversed vertex sequences first differ at the last
   vertex (the endpoint), so comparing (len, cnt, id) decides. */
typedef struct {
    double len;
    int    cnt;
    int    id;
...
/* ─── Constant-Degree Transform (paper §2) ──────────────────────────
...
typedef struct {
    ...
    int *cyc_by_nb;  /* [Σ nb_len] neighbor-list entry -> cycle vertex id */
} Transform;
...
/* k = floor(log^{1/3} n), t = floor(log^{2/3} n) as in the paper.
   ...
   top-level BMSSP call is always a successful execution (U = V, Lemma 3.6). */
...
typedef struct {
    ...
    int    *cnt;       /* α = number of vertices on the path */
    int     n;
    int     k, t;
} State;
...
/* Relax edge (u, v) with weight w. Always applies the min of the paper's
   update d̂[v] <- min{d̂[v], d̂[u]+w_uv}; a stale tail label (d̂[u] improved
   since v was last relaxed through u) must still propagate. Returns 1 if
   the relaxation is valid (strictly improves, or the recorded predecessor
   re-relaxes an unchanged label, Remark 3.4). */
...
/* AVL tree over the upper bounds of the non-empty D1 blocks (Lemma 3.3).
   A block's upper never increases after insertion into the tree (Insert only
   adds values no larger than the located block's upper, and the paper does
   not refresh uppers after deletes), so each block's tree key is fixed. */
typedef struct {
    int b;         /* block id */
    int l, r, p;
    int h;
...
/* ─── Selection (Blum et al. median-of-medians, O(n)) ────────────── */
...
static void blk_split(DPQ *q, int b) {
    ...
    /* median in O(cnt) [BFP+73]; elements < median to b1, rest to b2 */
...
static void dpq_batch_prepend(DPQ *q, const int *kk, const Lab *kv, int len) {
    ...
    /* 4. Chunk into blocks of at most ceil(M/2) (Lemma 3.3) and prepend to
          D0 so that blocks end up in ascending order (smallest at the head):
          build from the largest chunk backwards, each seq_prepend puts its
          block at the head. */
    int chunk = (q->M + 1) / 2;
    if (chunk < 1) chunk = 1;
    for (int i = m; i > 0; ) {
...
static int dpq_pull(DPQ *q, int *ok, Lab *ov, int M, Lab *bound) {
    ...
    /* Collect a prefix of whole blocks from each sequence separately, stopping
       each sequence once M elements from it have been taken (Lemma 3.3 Pull).
    ...
    if (n <= M) {
        ...
        /* Select the M smallest among the collected in O(M) [BFP+73]. */
...
static void find_pivots(State *s, Lab B,
    ...
    *Wlen = 0;
    ...
    /* Shortest-path forest over W: F = { (Pred[v], v) : v in W, Pred[v] in W }.
       Pred[] is maintained by relax() and always satisfies the paper's
       invariant, so trees follow the recorded predecessors directly. */
...
static void base_case(State *s, Lab B, const int *S, int Slen,
    ...
    while (h.n > 0 && local_len < k + 1) {
        ...
        for (int e = s->g->off[u]; e < s->g->off[u + 1]; e++) {
            ...
            nd.cnt = s->cnt[u] + 1;
            nd.id  = v;
            /* Non-strict: equal labels still propagate (Remark 3.4). */
    ...
    if (local_len <= k) {
        *Bp = B;
        ...
        /* B' = max label over U0 and trim exactly that vertex. The max label
           is unique under the total order (ties on length are broken by α
           and id), so U keeps the k closest vertices — the paper's base case. */
...
static void bmssp_recurse(State *s, int l, Lab B,
    ...
    while (!dpq_empty(&D)) {
        ...
        /* Drop stale entries of vertices already settled: a vertex becomes
           complete once it enters U, and Lemma 3.6 needs S_i to be exactly
           the pulled set of incomplete vertices. */
        {
            int w = 0;
        ...
        /* U is a set: count and keep each settled vertex once (Remark 3.8). */
    ...
    /* B' = B on success; on a partial exit it is the current recursion's
       bound B'_i (Algorithm 3, Lemma 3.9). */
...
int ssort(const SsortGraph *g, int src, double *dist, int *pred) {
    ...
    /* Run BMSSP on the constant-degree transformed graph (paper §2);
    ...
    /* BMSSP is exact end-to-end (Lemma 3.7): every reachable vertex ends up
    ...
    /* Back-map predecessors: pred[v] is the in-neighbor u minimizing
       ...
       and dist[u]+w_uv is recomputed with the same operands BMSSP used). */
